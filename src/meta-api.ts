...
export async function graphRequest<T = unknown>(
  ...
  const version = opts.apiVersion ?? process.env.META_GRAPH_VERSION ?? DEFAULT_VERSION;
...
export function requireEnvToken(): string {
  ...
  if (!t?.trim()) {
    throw new Error(
      "META_ACCESS_TOKEN is not set. Add it to your environment or MCP server config."
    );
  }
  return t.trim();
}
